"""
Caso 1/5: 07/04/2026 -- Fabian SELL 10:01 (Hedge/MER) vs codigo SELL 10:02
(1 min despues). Analisis exhaustivo a pedido de Diego (28/08/2026):
"vamos caso por caso... releé todo, medí velas, ve patrones, analiza todo".
"""
import pandas as pd
import numpy as np
import pytz
...
from prueba_ventana_horaria import cargar, ventana_ny, tipo_envolvente

NY = pytz.timezone('America/New_York')
UTC = pytz.UTC
VERDE, ROJO = '#26a69a', '#ef5350'
OUT = '/Users/diegorodriguez/Desktop/Jarvis/jarvis/trading_algoritmico/graficos/caso_07abr_fabian_vs_codigo.png'
...
if __name__ == '__main__':
    ...
    dia = pd.Timestamp('2026-04-07').date()
    g = df[df['day'] == dia]
    _, fin = ventana_ny(pd.Timestamp(dia), 9, 1, 10, 59)
    g_amplia = g[g.index <= fin]

    ini_vis, fin_vis = ventana_ny(pd.Timestamp(dia), 9, 54, 10, 9)
    ...
    for ax, t_entrada_str, titulo, anot in zip(
        axes,
        ['2026-04-07 10:01:00', '2026-04-07 10:02:00'],
        ['FABIAN (real) -- SELL 10:01 -- MER (Hedge)',
         'CÓDIGO -- SELL 10:02 -- MEC-A (continuación, 1 min después)'],
        ['10:01: rompe el nivel con la mecha (low 4634.96) pero\ncuerpo de la vela = 47,6% -- no llega al piso\nde 50% que exige la fórmula para contar como\nenvolvente/martillo válida',
         '10:02: ya no es "ruptura fresca" (el estado\nya había cambiado a bajista en 10:01) -- se toma\ncomo continuación (mecA) porque esta vela sí\ntiene cuerpo 74,2%, envolvente válida']
    ):
        ...
        ax.plot([0, len(g_vis) - 1], [nivel_bajo, nivel_bajo], color='#787b86', linestyle='--', linewidth=1.0, alpha=0.8)
        ax.text(len(g_vis) - 1, nivel_bajo, f'  nivel M3 {nivel_bajo:.2f}', color='#9aa0a8', fontsize=8, va='center')
