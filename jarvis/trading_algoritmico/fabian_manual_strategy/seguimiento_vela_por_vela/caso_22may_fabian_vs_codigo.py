"""
Caso 4/5: 22/05/2026 -- Fabian SELL 10:03 vs codigo SELL 10:00 (unico caso
donde el codigo va ANTES que Fabian). Analisis exhaustivo (28/08/2026).
"""
import pandas as pd
import numpy as np
import pytz
...
OUT = '/Users/diegorodriguez/Desktop/Jarvis/jarvis/trading_algoritmico/graficos/caso_22may_fabian_vs_codigo.png'
...
if __name__ == '__main__':
    ...
    dia = pd.Timestamp('2026-05-22').date()
    g = df[df['day'] == dia]
    _, fin = ventana_ny(pd.Timestamp(dia), 9, 1, 10, 59)
    g_amplia = g[g.index <= fin]

    ini_vis, fin_vis = ventana_ny(pd.Timestamp(dia), 9, 55, 10, 8)
    ...
    for ax, t_entrada_str, titulo, anot in zip(
        axes,
        ['2026-05-22 10:03:00', '2026-05-22 10:00:00'],
        ['FABIAN (real, hora declarada) -- SELL 10:03',
         'CÓDIGO -- SELL 10:00 -- MEC-A (3 min ANTES)'],
        ['10:03: vela ALCISTA (cuerpo 31,8%) -- no calza\ncomo vela de señal para un SELL. Hipótesis:\nla hora anotada por Fabian puede tener\nun margen de error humano acá',
         '10:00: vela bajista enorme, cuerpo 91,0% --\nrompe el nivel M3 recién nacido con fuerza\nclara. Podría ser el momento real de la\ndecisión de Fabian, anotado 3 min tarde']
    ):
