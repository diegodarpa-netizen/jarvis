"""
Descarga puntual del tramo que le falta al cruce de Fabian: 27/10/2025 a
12/02/2026 (el dia antes de donde arranca nuestro M1 de 6 meses). Reusa
el mismo motor ya validado de download_dukascopy_5y.py (resume + sin
repaint), pero acotado a este rango especifico para no esperar a que la
descarga de 5 años (secuencial, empieza en 2021) llegue hasta aca.
"""
import requests, struct, lzma, datetime, os, csv, time
import pytz

OUTPUT = os.path.join(os.path.dirname(__file__), 'XAUUSD_M1_gap_fabian.csv')
SYMBOL = "XAUUSD"
...
if __name__ == '__main__':
    ...
    if existentes:
        ...
        resume_from = START
        existentes = []
        print(f"Descargando gap Fabian: {START} -> {END}", flush=True)
    ...
    while current <= END:
        ...
        dias_done += 1
        if dias_done % 5 == 0:
            print(f"  Progreso: {current} ({dias_done} dias procesados, {len(all_bars)} velas)", flush=True)
            _save(all_bars)
